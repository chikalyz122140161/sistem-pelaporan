...
export const createTicket = async (user_id, title, description, category, priority) => {
  const query = `
    INSERT INTO tickets (user_id, title, description, category, priority, status)
    VALUES ($1, $2, $3, $4, $5, 'OPEN')
    RETURNING *
  `;
  const result = await pool.query(query, [user_id, title, description, category, priority]);
